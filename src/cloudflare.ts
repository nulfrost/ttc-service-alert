...
export async function writeDataToCloudflareKV({ timestamp, alerts }: { timestamp: string; alerts: string }) {
	try {
		...
		await reportErrorToDiscord({ title: 'could not write to cloudflare kv', context: JSON.stringify({ timestamp, alerts }) })
		throw error;
	}
}
...
export async function listKVKeys() {
	try {
		...
		await reportErrorToDiscord({ title: 'could not list kv keys', context: JSON.stringify(error) })
		throw error;
	}
}
...
export async function getValueByKey(key: string): Promise<string> {
	try {
		...
		await reportErrorToDiscord({ title: `could not get value for key ${key}`, context: key })
		throw error;
	}
}
...
export async function insertIds({ alert_id, threads_post_id }: { alert_id: number; threads_post_id: string }) {
	try {
		...
		await reportErrorToDiscord({ title: 'could not insert ID to database', context: JSON.stringify(({ alert_id, threads_post_id })) })
		throw error;
	}
}
...
export async function findTransitAlertById(alert_id: string) {
	try {
		...
		await reportErrorToDiscord({ title: `could not find alert for ID ${alert_id}`, context: alert_id })
		throw error;
	}
}
...
async function updatePaginatedCursor(cursor: string) {
	try {
		...
		await reportErrorToDiscord({ title: 'could update paginated cursor', context: cursor })
		throw error;
	}
}
...
async function getPaginatedCursor() {
	try {
		...
		await reportErrorToDiscord({ title: 'could not get paginated cursor', context: JSON.stringify(error) })
		throw error;
	}
}
