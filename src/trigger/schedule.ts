...
export const scheduledThreadsPost = schedules.task({
	...
	run: async () => {
		try {
			...
			const filteredAlerts = filterAlertsByAlertType([
				...alerts.routes,
				...alerts.accessibility,
			]);
			const alertsSortedByMostRecentTimestamp =
				sortAlertsByTimestamp(filteredAlerts);
			...
			const parsedRecentAlert = destringify(
				lastCachedAlertData as unknown as string,
			);

			const parsedRecentAlertIds = new Set(
				parsedRecentAlert.map((alert) => alert.id),
			);
			const newAlertsBasedOnIds = alertsSortedByMostRecentTimestamp.filter(
				(alert) => !parsedRecentAlertIds.has(alert.id),
			);

			if (newAlertsBasedOnIds.length === 0) {
				// no new alerts based on ids
				console.info(
					"no new alerts based on ids, checking for updates based on content",
				);

				const parsedRecentAlertTitles = new Set(
					parsedRecentAlert.map((alert) => alert.headerText),
				);
